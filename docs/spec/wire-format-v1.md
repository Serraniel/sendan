...
| Value | Size | Origin |
|---|---|---|
| File identifier `fileID` | 16 bytes | **client**, random |
...
> [!IMPORTANT]
> **`fileID` is generated by the client, and must be.** It is the salt in the
> key schedule (§4), so every key an upload has depends on it. A server-assigned
> identifier could not exist before the keys, and the keys cannot exist before
> the identifier — a client would have nothing to derive from when it builds the
> values creation requires.
>
> Do not change this back. The circularity is not obvious from §3 alone, which
> is why it is written here.
>
> The server validates the length and alphabet, rejects a duplicate, and rejects
> a value that is a single repeated byte. It cannot do more: see §13.
...
### 13.1 What a server cannot check

A server cannot tell a well-generated key from a badly generated one, and this
is a consequence of the design rather than an omission.

Everything it receives is either ciphertext or a digest. The wrapped file key
and the metadata envelope are AES-256-GCM output, which is indistinguishable
from random unless the cipher is broken; the token values arrive as SHA-256
digests, which are uniform. `FK`, `LS`, `KEK`, `MK` and the password never reach
it at all (invariant 4). A server able to judge the strength of a key would be a
server that had learned something about it.

The values arriving in the clear — `fileID`, the nonces, `pwSalt` — are too
short for any statistical test to say anything meaningful. Sixteen bytes cannot
be distinguished from the output of a good generator, so a check would be
guesswork with both false positives and false negatives.

What is worth rejecting is the degenerate case: a value that is one byte
repeated. That is not a weak generator, it is an absent one — an uninitialised
buffer or a stub that returns zeroes. It is cheap to detect and catches a real
class of implementation fault, and it should not be mistaken for a defence
against a client that has decided to be weak.

Such a client harms only its own uploads. It cannot weaken anyone else's, and
nothing it does affects an upload made by a correct client. A user's protection
against a client that is subtly wrong is knowing which client they are running,
which is the verification described in `docs/design.md` §7.1.
